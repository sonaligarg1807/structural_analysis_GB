# version which calculates misorientation axis alignment with GB normal  calculated from respective gb.gro file
...
ENHANCED:
  - Misorientation axis, twist/tilt decomposition.
  - Alignment of misorientation axis with GB normal (from GB.gro when available).
  - Alignment of misorientation axis with a/b/c axes of each grain.
  - GB type classification: twist / tilt / mixed.
"""
...
# ========= Optional: MDAnalysis for GB.gro handling =========
try:
    import MDAnalysis as mda  # type: ignore

    MDA_AVAILABLE = True
    logger.info("[misori] MDAnalysis available for GB.gro-based GB normals.")
except Exception as e:  # pragma: no cover - import-time may fail
    MDA_AVAILABLE = False
    logger.info(
        "[misori] MDAnalysis not available (%s). Falling back to COM-based GB normal if needed.",
        e,
    )
...
# ----------------------------------------------------------------------
# NEW: GB normal from GB.gro via PCA
# ----------------------------------------------------------------------
def gb_normal_from_gb_gro(gb_gro_file: str | Path) -> Optional[np.ndarray]:
    """
    Compute a GB plane normal from a GB.gro file using PCA on GB residue COMs.

    The logic mirrors the Step-3 local frame construction:
      - Compute COM of each residue in GB.gro.
      - PCA → eigenvectors v0 (min var), v1, v2 (max var).
      - v0 → slab-normal candidate n_slab.
      - v1 projected into the slab plane → GB-normal candidate n_gb.

    Parameters
    ----------
    gb_gro_file : str or Path
        Path to the GB-only .gro file for a given segment (Step-3 output).

    Returns
    -------
    n_gb : np.ndarray or None
        Unit vector for GB plane normal in lab coordinates, or None if failed.
    """
    if not MDA_AVAILABLE:
        logger.warning(
            "[misori] MDAnalysis not available; cannot compute GB normal from GB.gro."
        )
        return None

    gb_gro_file = Path(gb_gro_file)
    if not gb_gro_file.is_file():
        logger.warning("[misori] GB.gro file not found: %s", gb_gro_file)
        return None

    try:
        u = mda.Universe(str(gb_gro_file))
    except Exception as e:
        logger.exception("[misori] Failed to load GB.gro with MDAnalysis: %s", e)
        return None

    coms = np.array([res.atoms.center_of_mass() for res in u.residues], float)
    if coms.shape[0] < 3:
        logger.warning(
            "[misori] Not enough GB residues (%d) in GB.gro to define a plane.",
            coms.shape[0],
        )
        return None
    ...
    v1 = vecs[:, 1]  # GB-normal candidate in slab plane

    n_slab = unit(v0)
    n_gb_raw = unit(v1)

    # Enforce GB normal in slab plane (orthogonal to n_slab)
    n_gb = n_gb_raw - np.dot(n_gb_raw, n_slab) * n_slab
    n_gb = unit(n_gb)

    logger.info(
        "[misori] GB frame from GB.gro %s: n_slab=%s  n_gb=%s",
        gb_gro_file.name,
        np.round(n_slab, 4),
        np.round(n_gb, 4),
    )
...
# ----------------------------------------------------------------------
def find_closest_axis(
    misori_axis: np.ndarray, a: np.ndarray, b: np.ndarray, c: np.ndarray
) -> Tuple[str, float]:
    """
    Find which crystallographic axis (a, b, or c) is most aligned with the
    misorientation axis.

    Returns (axis_label, angle_deg).
    """
    ...
    angle_deg = float(np.degrees(np.arccos(np.clip(dots[closest], -1.0, 1.0))))

    logger.debug(
        "[misori] Axis alignment (deg): a=%.2f, b=%.2f, c=%.2f → closest=%s",
        float(np.degrees(np.arccos(np.clip(dot_a, -1, 1)))),
        float(np.degrees(np.arccos(np.clip(dot_b, -1, 1)))),
        float(np.degrees(np.arccos(np.clip(dot_c, -1, 1)))),
        closest,
    )

    return closest, angle_deg


def calculate_axis_gb_normal_angle(
    misori_axis: np.ndarray, gb_normal: np.ndarray
) -> float:
    """
    ...
    gb_normal : np.ndarray
        Unit vector representing the GB plane normal in lab coordinates.

    Returns
    -------
    angle_deg : float
        Angle in degrees between the two vectors (0–90°, using |dot|).
    """
    misori_axis = unit(misori_axis)
    gb_normal = unit(gb_normal)

    dot_product = abs(np.dot(misori_axis, gb_normal))
    angle_deg = float(np.degrees(np.arccos(np.clip(dot_product, -1.0, 1.0))))
...
def classify_gb_type(
    twist_deg: float,
    tilt_deg: float,
    theta_deg: float,
    axis_gb_angle: Optional[float] = None,
    twist_threshold: float = 15.0,
    tilt_threshold: float = 75.0,
) -> str:
    """
    Classify grain boundary as twist-dominated, tilt-dominated, or mixed.

    If axis_gb_angle is available:
      - angle <  twist_threshold → 'twist'
      - angle >  tilt_threshold  → 'tilt'
      - else                     → 'mixed'

    If axis_gb_angle is None, falls back to component fractions.
    """
    if theta_deg < 0.1:
        return "NA"
    ...
    if axis_gb_angle is not None:
        if axis_gb_angle < twist_threshold:
            ...
            logger.debug(
                "[misori] Classification: twist (axis || GB normal, angle=%.2f°)",
                axis_gb_angle,
            )
        elif axis_gb_angle > tilt_threshold:
            classification = "tilt"
            logger.debug(
                "[misori] Classification: tilt (axis ⟂ GB normal, angle=%.2f°)",
                axis_gb_angle,
            )
        else:
            classification = "mixed"
            logger.debug(
                "[misori] Classification: mixed (axis-GB angle=%.2f°)",
                axis_gb_angle,
            )
    else:
        twist_fraction = twist_deg / theta_deg if theta_deg > 0 else 0.0
        tilt_fraction = tilt_deg / theta_deg if theta_deg > 0 else 0.0

        if twist_fraction > 0.8:
            classification = "twist"
            logger.debug(
                "[misori] Classification: twist (twist/total=%.2f)", twist_fraction
            )
        elif tilt_fraction > 0.8:
            classification = "tilt"
            logger.debug(
                "[misori] Classification: tilt (tilt/total=%.2f)", tilt_fraction
            )
        else:
            classification = "mixed"
            logger.debug(
                "[misori] Classification: mixed (twist=%.2f, tilt=%.2f)",
                twist_fraction,
                tilt_fraction,
            )
...
# ----------------------------------------------------------------------
# Core misorientation computation via orix
# ----------------------------------------------------------------------
def calculate_misorientation_orix(
    ...
    try:
        ...
        if sym is None:
            logger.warning(
                "[misori] Could not build symmetry; skipping misorientation."
            )
            return None, None, None
...
# ----------------------------------------------------------------------
# Misorientation from frames + GB normal
# ----------------------------------------------------------------------
def misorientation_from_frames(
    ...
    gb_normal : optional 3-vector
        GB plane normal in lab coords. If provided, compute twist/tilt
        decomposition and axis alignment analysis.
    symmetry_name : str
        Symmetry label for orix.

    Returns
    -------
    dict with keys (floats or None):
      - "theta_deg" : misorientation angle
      - "axis" : 3-vector misorientation axis
      - "twist_deg", "tilt_deg"
      - "method" : string
      - "axis_gb_normal_angle"
      - "axis_g1_closest", "axis_g1_angle"
      - "axis_g2_closest", "axis_g2_angle"
      - "dominant_type"
    """
    ...
    axis_gb_angle: Optional[float] = None
    axis_g1_closest: Optional[str] = None
    axis_g1_angle: Optional[float] = None
    axis_g2_closest: Optional[str] = None
    axis_g2_angle: Optional[float] = None
    dominant_type: str = "NA"

    if gb_normal is not None:
        gbn = unit(gb_normal)

        # Twist/tilt decomposition in terms of misorientation axis vs GB normal
        twist_deg = abs(float(np.dot(axis, gbn))) * angle_deg
        tilt_sq = max(angle_deg**2 - twist_deg**2, 0.0)
        tilt_deg = float(np.sqrt(tilt_sq))

        # Axis–GB normal alignment
        axis_gb_angle = calculate_axis_gb_normal_angle(axis, gbn)

        # Axis alignment with each grain frame
        ...
        # GB type classification
        dominant_type = classify_gb_type(
            twist_deg, tilt_deg, angle_deg, axis_gb_angle
        )
...
# ----------------------------------------------------------------------
# High-level wrapper for your GB pipeline
# ----------------------------------------------------------------------
def misorientation_for_group(
    ...
    symmetry_name: str = "triclinic",
    gb_gro_file: Optional[str | Path] = None,
) -> Dict[str, Optional[float | np.ndarray | str]]:
    """
    High-level convenience wrapper for the GB pipeline.

    Mirrors contactplanes_for_group inputs and returns the same dict format
    as misorientation_from_frames(), with an additional option:

    Parameters
    ----------
    g1_gro_file, g2_gro_file : str or Path
        Grain-1 and Grain-2 .gro files (Step-3 segment outputs).
    g1_txt, g2_txt : str or Path, optional
        Corresponding *_latvecs.txt files. If None, inferred with
        default_latvec_output_path().
    symmetry_name : str
        Crystal symmetry label for orix.
    gb_gro_file : str or Path, optional
        GB-only .gro file for this segment (Step-3 output). If provided,
        the GB plane normal will be computed from GB.gro via PCA and used
        for twist/tilt decomposition and axis alignment. If not provided
        or fails, falls back to COM-based connection vector between
        g1 and g2 as a rough GB normal.
    """
    g1_gro_file = Path(g1_gro_file)
    g2_gro_file = Path(g2_gro_file)

    # Default latvec txt paths (consistent with contact-plane logic)
    if g1_txt is None:
    ...
    # ------------------------------------------------------------------
    # GB normal: prefer GB.gro → fallback to COM-based connection vector
    # ------------------------------------------------------------------
    gb_normal: Optional[np.ndarray] = None

    if gb_gro_file is not None:
        gb_normal = gb_normal_from_gb_gro(gb_gro_file)

    if gb_normal is None:
        # Fallback: use COM-based connection vector between grain chunks
        com1 = compute_com_from_gro(g1_gro_file)
        com2 = compute_com_from_gro(g2_gro_file)
        conn_vec = com2 - com1
        if norm(conn_vec) < 1e-10:
            logger.error("[misori] Grains overlap, cannot compute fallback GB normal.")
            return {
                "theta_deg": None,
                "axis": None,
                "twist_deg": None,
        ...
        logger.warning(
            "[misori] Using COM-based connection vector as GB normal fallback."
        )

    # ------------------------------------------------------------------
    # Lattice vectors and frames
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Misorientation + twist/tilt relative to GB normal
    # ------------------------------------------------------------------
    ...
    if miso["theta_deg"] is not None:
        ...
        logger.info(
            "[misori] Misorientation not computed (orix missing or input issue)."
        )
